...
spark = SparkSession.builder.appName("sparkpyrediskafkastream").getOrCreate()
...
kafkaStreamingDF.withColumn("value",from_json("value",redisMessageSchema)).select(col('value.*')).createOrReplaceTempView("RedisSortedSet")


# TO-DO: execute a sql statement against a temporary view, which statement takes the element field from the 0th element in the array of structs  
...
#zSetEntriesEncodedStreamingDF.writeStream.outputMode("append").format("console").start().awaitTermination()
...
zSetDecodedEntriesStreamingDF.withColumn("encodedCustomer",from_json("encodedCustomer", customerjson)).select(col("encodedCustomer.*")).createOrReplaceTempView("CustomerRecords")
#
